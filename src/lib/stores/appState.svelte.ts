...
class AppState {
  private mdConverter = createMarkdownConverter();
  ...
  showMarkdownImportModal = $state<boolean>(false);
  ...
  showConflictResolver = $state<boolean>(false);
  private conflictResolveCallback: ((resolutions: Array<{ notePath: string; choice: 'local' | 'remote' | 'both' }>) => void) | null = null;
  ...
  // List all conflict copies and deleted backups for recovery
  async listConflictFiles(): Promise<NoteFile[]> {
    const allNotes = await this.storage.listNotes();
    return allNotes.filter(n => n.path.startsWith('.conflicts/'));
  }

  // Restore a conflict file back as a normal note (moves from .conflicts/ to root)
  async restoreConflictFile(conflictPath: string, restoreAsPath?: string): Promise<void> {
    const allNotes = await this.storage.listNotes();
    const conflictNote = allNotes.find(n => n.path === conflictPath);
    if (!conflictNote) throw new Error('Conflict file not found');

    // Derive a restore path if not provided
    const targetPath = restoreAsPath || conflictPath
      .replace(/^\.conflicts\//, '')
      .replace(/\s*\((local|remote|deleted)\s+\d{4}-\d{2}-\d{2}\)/, '');

    await this.storage.writeNote(targetPath, conflictNote.content);
    await this.storage.deleteNote(conflictPath);
    await this.refreshNotes();
    this.showToast(`Restored conflict file as "${targetPath}".`, 'success', 4000);
  }

  // Delete a conflict file permanently
  async deleteConflictFile(conflictPath: string): Promise<void> {
    await this.storage.deleteNote(conflictPath);
  }

  // Resolve pending sync conflicts — called from ConflictResolver UI
  resolveConflicts(resolutions: Array<{ notePath: string; choice: 'local' | 'remote' | 'both' }>) {
    if (this.conflictResolveCallback) {
      this.conflictResolveCallback(resolutions);
  ...
  // Wait for user to resolve conflicts via the UI
  private waitForConflictResolution(): Promise<Array<{ notePath: string; choice: 'local' | 'remote' | 'both' }>> {
  ...
  async syncNotes() {
    ...
    // Collect genuine conflicts for user resolution
    const deferredConflicts: Array<{
      notePath: string;
      noteName: string;
      localContent: string;
      remoteContent: string;
    ...
    try {
        ...
        if (driveFile) {
          ...
          if (!mapEntry) {
            ...
            if (remoteContent !== null && this.notesBodyEqual(note.content, remoteContent)) {
              ...
              // Remote is newer but content differs — save local as conflict copy before overwriting
              if (!this.notesBodyEqual(note.content, remoteContent)) {
                const noteName = note.path.split('/').pop()?.replace(/\.(html|md)$/, '') || note.name;
                const conflictDate = new Date().toISOString().slice(0, 10);
                const ext = note.path.endsWith('.md') ? '.md' : '.html';
                const conflictPath = `.conflicts/${noteName} (local ${conflictDate})${ext}`;
                try {
                  await this.storage.writeNote(conflictPath, note.content);
                  console.log(`  First sync: saved local as conflict copy: ${conflictPath}`);
                  this.showToast(`First sync: "${noteName}" — remote was newer. Your local version saved as conflict copy.`, 'warning', 8000);
                } catch (backupErr) {
                  // Cannot save backup — abort overwrite for safety
                  console.error('  CRITICAL: Failed to save conflict backup during first sync. Aborting overwrite.', backupErr);
                  activeMappings[note.path] = {
                    id: driveFile.id,
                    lastSyncLocalTime: 0,
                    lastSyncRemoteTime: 0
                  };
                  this.showToast(`First sync: could not back up "${noteName}". Skipping to protect your data.`, 'error', 8000);
                  continue;
                }
              }
          ...
          if (localChanged && remoteChanged) {
            ...
            // Always download remote and compare content before deciding.
            console.log('  Conflict detected! Downloading remote to compare content...');
            let remoteContent: string | null = null;
            try {
              remoteContent = await this.syncService.downloadFile(driveFile.id);
            } catch (e) {
              console.warn('  Failed to download remote for conflict resolution:', e);
            }

            if (remoteContent !== null && this.notesBodyEqual(note.content, remoteContent)) {
              // Content is actually identical — no real conflict, just timestamp drift.
              console.log('  Content is identical despite timestamp difference. No conflict.');
              activeMappings[note.path] = {
                id: driveFile.id,
                lastSyncLocalTime: note.modified,
                lastSyncRemoteTime: remoteTime
              ...
              // Genuine conflict — queue for user resolution
              const noteName = note.path.split('/').pop()?.replace(/\.(html|md)$/, '') || note.name;
              console.log(`  Genuine conflict on "${noteName}". Queuing for user resolution.`);
              ...
              // Temporarily preserve old mapping so nothing is lost
              activeMappings[note.path] = {
                id: driveFile.id,
                lastSyncLocalTime: lastSyncLocalTime,
                lastSyncRemoteTime: lastSyncRemoteTime
              };
            } else {
              // Could not download remote — fallback: upload local to avoid stalling sync
              console.log('  Cannot resolve conflict (remote download failed). Uploading local as fallback.');
              const filename = note.path.split('/').pop() || `${note.name}.html`;
              const uploadRes = await uploadNoteToDrive(note.path, filename, note.content, driveFile.id);
              const updatedRemoteTime = new Date(uploadRes.modifiedTime).getTime();
              activeMappings[note.path] = {
          ...
          if (remoteId) {
            // Remote file not found on Drive, but we have a mapping.
            // Could be: intentional Drive deletion, or Drive folder change, or API issue.
            // Safety: only delete locally if note hasn't been modified since last sync.
            // If it was modified locally, keep it and re-upload (user was working on it).
            const mapEntry2 = mappings[note.path];
            const lastSyncLocal2 = (mapEntry2 && typeof mapEntry2 !== 'string') ? mapEntry2.lastSyncLocalTime : 0;
            const localModifiedSinceSync = note.modified > lastSyncLocal2 + 2000;

            if (localModifiedSinceSync) {
              // Note was edited locally after last sync — do NOT delete. Re-upload instead.
              console.log('  Remote file gone, but local was modified since last sync. Re-uploading to preserve local edits.');
              const filename = note.path.split('/').pop() || `${note.name}.html`;
              ...
              // Note wasn't modified since last sync — safe to assume intentional deletion.
              // Still save a backup before deleting, as an extra safety net.
              console.log('  Note was deleted on Google Drive. Saving backup then deleting locally...');
              const noteName = note.path.split('/').pop()?.replace(/\.(md|html)$/, '') || note.path;
              const ext = note.path.endsWith('.md') ? '.md' : '.html';
              const backupPath = `.conflicts/${noteName} (deleted ${new Date().toISOString().slice(0, 10)})${ext}`;
              try {
                await this.storage.writeNote(backupPath, note.content);
              } catch (e) {
                console.warn('  Failed to create backup before deletion:', e);
              }
              try {
                await this.storage.deleteNote(note.path);
                this.showToast(`Deleted note "${noteName}" locally to match Google Drive deletion. Backup saved.`, 'info', 4000);
              } catch (err) {
      ...
      for (const remote of driveFiles) {
        ...
        if (oldLocalPath) {
          ...
          // Safety check: only delete remote if the mapping is recent (last sync was within 30 days).
          // If mappings are very old (e.g., localStorage was cleared and recreated, or a long gap),
          // download instead of deleting to prevent accidental data loss.
          const oldMap = mappings[oldLocalPath];
          const lastSyncTime = (oldMap && typeof oldMap !== 'string') ? oldMap.lastSyncLocalTime : 0;
          const daysSinceLastSync = (Date.now() - lastSyncTime) / (1000 * 60 * 60 * 24);

          if (lastSyncTime > 0 && daysSinceLastSync < 30) {
            // Recent sync record — confident this was an intentional local deletion.
            console.log(`Note ${oldLocalPath} was deleted locally (last synced ${daysSinceLastSync.toFixed(1)} days ago). Deleting remote file ${remote.path} (ID: ${remote.id}) on Drive...`);
            try {
            ...
            // Stale or missing sync timestamp — unsafe to delete. Download instead.
            console.log(`Note ${oldLocalPath} not found locally, but sync record is stale (${daysSinceLastSync.toFixed(0)} days). Downloading remote instead of deleting.`);
      ...
      // Step 3: Resolve deferred conflicts with user input
      if (deferredConflicts.length > 0) {
        console.log(`  ${deferredConflicts.length} conflict(s) require user resolution. Pausing sync...`);
        this.pendingConflicts = deferredConflicts;
        const resolutions = await this.waitForConflictResolution();
        
        const conflictDate = new Date().toISOString().slice(0, 10);
        for (const resolution of resolutions) {
          const conflict = deferredConflicts.find(c => c.notePath === resolution.notePath);
          if (!conflict) continue;

          const ext = conflict.notePath.endsWith('.md') ? '.md' : '.html';
          const filename = conflict.notePath.split('/').pop() || `${conflict.noteName}.html`;

          if (resolution.choice === 'local') {
            // Keep local, upload to Drive, save remote as backup
            console.log(`  Conflict resolved: keeping local for "${conflict.noteName}"`);
            const conflictPath = `.conflicts/${conflict.noteName} (remote ${conflictDate})${ext}`;
            try { await this.storage.writeNote(conflictPath, conflict.remoteContent); } catch {}
            const uploadRes = await uploadNoteToDrive(conflict.notePath, filename, conflict.localContent, conflict.driveFileId);
            const updatedRemoteTime = new Date(uploadRes.modifiedTime).getTime();
            activeMappings[conflict.notePath] = {
              id: uploadRes.id,
              lastSyncLocalTime: conflict.localModified,
              lastSyncRemoteTime: updatedRemoteTime
            };
          } else if (resolution.choice === 'remote') {
            // Keep remote, save local as backup
            console.log(`  Conflict resolved: keeping remote for "${conflict.noteName}"`);
            const conflictPath = `.conflicts/${conflict.noteName} (local ${conflictDate})${ext}`;
            try { await this.storage.writeNote(conflictPath, conflict.localContent); } catch {}
            await this.storage.writeNote(conflict.notePath, conflict.remoteContent);
            activeMappings[conflict.notePath] = {
              id: conflict.driveFileId,
              lastSyncLocalTime: Date.now(),
              lastSyncRemoteTime: conflict.remoteModified
            };
          } else if (resolution.choice === 'both') {
            // Keep both — local stays as main, remote saved as separate note
            console.log(`  Conflict resolved: keeping both for "${conflict.noteName}"`);
            const conflictPath = `.conflicts/${conflict.noteName} (remote ${conflictDate})${ext}`;
            try { await this.storage.writeNote(conflictPath, conflict.remoteContent); } catch {}
            // Upload local to Drive since it's the "main" version
            const uploadRes = await uploadNoteToDrive(conflict.notePath, filename, conflict.localContent, conflict.driveFileId);
            const updatedRemoteTime = new Date(uploadRes.modifiedTime).getTime();
            activeMappings[conflict.notePath] = {
              id: uploadRes.id,
            ...
            this.showToast(`"${conflict.noteName}": both versions saved. Remote copy in Conflicts folder.`, 'info', 5000);
          }
        }
      }
  ...
  async migrateOldNotes() {
    if (this.storage.migrateNotes) {
      const convertFn = (md: string) => {
        ...
        return this.convertMarkdownToHtml(parsed.meta.title || 'Untitled', md);
      };
  ...
  async selectNote(path: string) {
    ...
    if (note) {
      ...
      if (this.activeTab === 'focus' || this.activeTab === 'budget') {
        this.activeTab = 'home';
      }
    }
  }
  ...
  convertMarkdownToHtml(title: string, markdown: string): string {
    ...
    const parsed = parseFrontmatter(preprocessed);
    
    // Retrieve configuration variables from localStorage (matching Editor settings)
    const activeVault = localStorage.getItem('mynotes_active_vault') || 'Local Sandbox';
    const enableWikiLinks = localStorage.getItem('mynotes_enable_wiki_links') !== 'false';
    const hideTitleInBody = localStorage.getItem('mynotes_hide_title_in_body') !== 'false';
    const pdfHeight = Number(localStorage.getItem('mynotes_pdf_height')) || 600;
    const pdfPreview = localStorage.getItem('mynotes_pdf_preview') !== 'false';

    // Populate wikiLinkTitlesCache
    const wikiLinkTitlesCache = this.notes.map(n => ({ title: n.name, path: n.path }));

    const htmlBody = this.mdConverter.markdownToHtml(parsed.content, {
      enableWikiLinks,
      wikiLinkTitlesCache,
      ...
      pdfPreview,
      hideTitleInBody,
      activeNoteTitle: parsed.meta.title || title
    });

    // Use the parsed title if present, otherwise fall back to cleanTitle
    const metaTitle = parsed.meta.title || title || 'Untitled';
    const meta = {
      ...parsed.meta,
      title: metaTitle,
      id: parsed.meta.id || '',
      created: parsed.meta.created || new Date().toISOString(),
      modified: parsed.meta.modified || new Date().toISOString()
    };
    return generateHtmlNote(meta, htmlBody);
  }

  async createNoteFromHtml(title: string, htmlContent: string, folder: string | null = null) {
    const cleanTitle = title.trim() || 'Untitled';
    let path = `${cleanTitle}.html`;
    if (folder) {
      path = `${folder}/${cleanTitle}.html`;
    ...
    // Since metadata is stored inside the HTML head, let's update the id/title inside the metadata in htmlContent
    let finalContent = htmlContent;
    try {
      const parsed = parseHtmlMetadata(htmlContent);
      parsed.meta.id = finalPath;
      parsed.meta.title = cleanTitle;
      finalContent = generateHtmlNote(parsed.meta, parsed.content);
    } catch (e) {
      console.warn('Failed to rewrite note metadata during creation from HTML:', e);
    }

    await this.storage.writeNote(finalPath, finalContent);
    await this.refreshNotes();
    await this.selectNote(finalPath);
  ...
  async createNoteFromUploadedHtml(filename: string, rawHtml: string, folder: string | null = null) {
    const cleanTitle = filename.replace(/\.html?$/i, '').trim() || 'Untitled';
    ...
    const sanitizeBody = (html: string): string => {
      if (typeof DOMParser !== 'undefined') {
        try {
          ...
          console.warn('DOMParser sanitization failed:', e);
        }
      }
      return html
    ...
    try {
      const parsed = parseHtmlMetadata(rawHtml);
      // Check if it has metadata or was created by MyNotes
      if (parsed.meta && (parsed.meta.id || parsed.meta.title || parsed.meta.created)) {
        parsedMeta = parsed.meta;
        if (parsed.meta.title) parsedTitle = parsed.meta.title;
        // Sanitize the content body to remove script and style tags
        const sanitizedContent = sanitizeBody(parsed.content);
        finalHtml = generateHtmlNote(parsedMeta, sanitizedContent);
      }
    } catch (e) {
      console.warn('Failed to parse HTML metadata, importing as raw HTML:', e);
    }
    ...
    if (!parsedMeta) {
      ...
      if (typeof DOMParser !== 'undefined') {
        ...
        bodyContent = sanitizeBody(rawHtml);
      }
      finalHtml = generateHtmlNote(parsedMeta, bodyContent);
    }

    await this.createNoteFromHtml(parsedTitle, finalHtml, folder);
  }
  ...
  async renameNote(oldPath: string, newTitle: string) {
    ...
    const ext = oldPath.split('.').pop() || 'html';
    const parts = oldPath.split('/');
    ...
    try {
      ...
      if (existing) {
        if (oldPath.endsWith('.html')) {
          ...
          await this.storage.writeNote(oldPath, updatedContent);
        } else if (oldPath.endsWith('.notebook.json')) {
          try {
            ...
            console.error('Failed to parse notebook JSON during rename', e);
          }
        }
      }
  ...
  async setCalcTagBudget(tagId: string, budget: number): Promise<void> {
    if (!this.calcTagDb) throw new Error('Calculation database not initialized');
    await this.calcTagDb.setTagBudget(tagId, budget);
    await this.refreshCalcTags();
  }
              ...
              for (const remoteTag of remoteTags) {
                ...
                if (!exists) {
                  const newTag: CalcTag = {
                    ...
                    budget: typeof remoteTag.budget === 'number' ? remoteTag.budget : 0
                  };
