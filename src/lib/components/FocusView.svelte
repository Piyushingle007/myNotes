            ...
            <div class="plan-list">
              {#if todayPlan.length > 0}
                <div class="plan-section">
                  ...
                  {#each todayPlan as card, i (card.id)}
                    <!-- svelte-ignore a11y_no_static_element_interactions -->
                    <div class="list-row" class:drag-over={dragOverIdx === i && dragReorderIdx !== i} style="--row-accent: {priorityColor(card.priority)};" draggable={!appState.isMobile} ondragstart={(e) => onDragStart(e, i)} ondragover={(e) => onDragOver(e, i)} ondrop={(e) => onDrop(e, i)} ondragend={onDragEnd} oncontextmenu={(e) => { if (!appState.isMobile) openContextMenu(e, card.id); }} use:longpress onlongpress={(e) => openMobileActionSheet(e.detail, card.id)} use:swipeActions={makeSwipeOpts(card)} in:fly={{ y: 12, duration: 250, delay: i * 40, easing: cubicOut }} out:fly={{ x: -30, duration: 200, easing: cubicOut }} animate:flip={{ duration: 250, easing: cubicOut }}>
              ...
              {#if queue.length > 0}
                <div class="plan-section">
                  ...
                  {#each queue as card (card.id)}
                    <!-- svelte-ignore a11y_no_static_element_interactions -->
                    <div class="list-row" style="--row-accent: {priorityColor(card.priority)};" oncontextmenu={(e) => { if (!appState.isMobile) openContextMenu(e, card.id); }} use:longpress onlongpress={(e) => openMobileActionSheet(e.detail, card.id)} use:swipeActions={makeSwipeOpts(card)} in:fly={{ y: 10, duration: 220, easing: cubicOut }} out:slide={{ duration: 180, easing: cubicOut }} animate:flip={{ duration: 250, easing: cubicOut }}>
