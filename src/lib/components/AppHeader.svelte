<script lang="ts">
  ...
  function togglePopover(name: 'noteList' | 'favorites' | 'notebooks' | 'tags' | 'dailyLogs') {
    console.log('[togglePopover] name:', name, 'current activePopover:', activePopover);
    if (activePopover === name) {
      activePopover = null;
    } else {
      activePopover = name;
      if (appState.activeTab === 'focus' || appState.activeTab === 'budget') {
  ...
  function handlePopoverOutsideClick(e: MouseEvent) {
    console.log('[handlePopoverOutsideClick] target:', e.target, 'popoverContainer:', popoverContainer);
    if (activePopover && popoverContainer && !popoverContainer.contains(e.target as Node)) {
      const target = e.target as HTMLElement;
      if (target.closest('.nav-icon-btn')) {
        console.log('[handlePopoverOutsideClick] ignored because target is closest to nav-icon-btn');
      ...
      console.log('[handlePopoverOutsideClick] closing popover because clicked outside');
      activePopover = null;
    }
  }
...
<header class="app-header flex-row">
  ...
  <div class="header-left flex-row">
    <div class="logo-group flex-row" onclick={() => { activePopover = null; showCreateDropdown = false; showSyncPopover = false; window.location.hash = '#/home'; }} role="button" tabindex="0" onkeydown={(e) => { if (e.key === 'Enter') { activePopover = null; showCreateDropdown = false; showSyncPopover = false; window.location.hash = '#/home'; } }}>
      <GoogleLogo size={22} />
    ...
    <div class="panel-toggles flex-row" style="gap: var(--spacing-xs);">
      <div class="desktop-nav-popovers flex-row" style="gap: 2px;">
        ...
        <button 
          class="panel-toggle-btn flex-row nav-icon-btn" 
          class:active={appState.activeTab === 'budget'} 
          onclick={() => { appState.activeTab = appState.activeTab === 'budget' ? 'home' : 'budget'; activePopover = null; }}
          title="Budget"
        >
      ...
      {#if showCreateDropdown}
        ...
        >
          ...
          <button 
            ...
            onmouseout={(e) => e.currentTarget.style.backgroundColor = 'transparent'}
          >
            <FileText size={16} style="color: var(--accent); flex-shrink: 0;" />
            <div class="flex-col" style="align-items: flex-start; gap: 2px;">
              <span style="font-weight: 600;">Import / Convert Markdown</span>
              <span style="font-size: var(--font-size-2xs); color: var(--text-tertiary); white-space: nowrap;">Import MD file or paste Markdown code</span>
            </div>
          </button>
        </div>
      {/if}
