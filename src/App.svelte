<script lang="ts">
  ...
  function syncStateToHash() {
    ...
    if (window.location.hash !== newHash) {
      ...
      setTimeout(() => {
        ignoreHashSync = false;
      }, 50);
    }
  }

  async function syncHashToState() {
    if (ignoreHashSync) return;
    ignoreHashSync = true;
    ...
    try {
      ...
      if (hash.startsWith('#/note/')) {
        ...
        if (appState.activeNotePath !== notePath) {
          await appState.selectNote(notePath);
        }
        ...
        if (appState.activeNotePath !== null) {
          ...
          await mobileNav.flushPendingSave();
